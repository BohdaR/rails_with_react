  ...
  private
    def permissions_handler(role, permission, employee)
      ...
      company_subjects_list = {
        ...
        role: { subject: Role, condition: { id: Role.joins(:auth_group).where("priority > ?", role.auth_group.priority).ids } },
        permission: { subject: Permission, condition: { id: Permission.joins(:auth_group).where("priority > ?", permission.auth_group.priority).ids } },
